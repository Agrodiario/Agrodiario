...
export class TranslationService {
  ...
  async translateToPortuguese(text: string): Promise<string> {
    ...
    this.logger.error(
      `✗ Translation failed for "${text}" after ${maxAttempts} attempts: ${lastError?.response?.data?.error?.message || lastError?.message}`,
    );
  ...
  async translateBatch(texts: string[]): Promise<string[]> {
    ...
    try {
      ...
      for (let i = 0; i < uniqueTexts.length; i += batchSize) {
        ...
        this.logger.debug(
          `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(uniqueTexts.length / batchSize)}`,
        );
        ...
        try {
          ...
          this.logger.error(
            `Batch API failed, falling back to individual translations: ${error.response?.data?.error?.message || error.message}`,
          );
      ...
      const translatedCount = result.filter(
        (t, i) => t.toLowerCase() !== texts[i].toLowerCase(),
      ).length;
      this.logger.log(
        `✓ Batch translation complete: ${translatedCount}/${texts.length} items translated`,
      );

      return result;
    } catch (error) {
      this.logger.error(
        `✗ Batch translation error: ${error.response?.data?.error?.message || error.message}`,
      );
