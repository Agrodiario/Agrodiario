...
class ProductApplicationService {
  async create(
    data: ProductApplicationFormData,
  ): Promise<ProductApplicationFormData> {
    const response = await apiClient.post<ProductApplication>(
  ...
  async findAll(
    ...
    const response = await apiClient.get<ProductApplicationsListResponse>(
      "/product-applications",
      {
    ...
    if (response.data.data.length > 0 && !response.data.data[0].propertyName) {
      const applicationsWithDetails = await this.enrichApplications(
        response.data.data,
      );
      return {
        ...response.data,
  ...
  ): Promise<ProductApplicationWithDetails[]> {
    try {
      // Buscar propriedades
      const propertiesResponse = await apiClient.get<{ data: Property[] }>(
        "/properties",
        {
      ...
      const propertiesMap = new Map(
        propertiesResponse.data.data.map((p) => [p.id, p.name]),
      );

      // Buscar culturas
      const culturesResponse = await apiClient.get<{ data: Culture[] }>(
        "/cultures",
        {
          params: { limit: 1000 }, // Buscar todas
        },
      ...
      const culturesMap = new Map(
        culturesResponse.data.data.map((c) => [c.id, c.cultureName]),
      );
  ...
  async findOne(id: string): Promise<ProductApplicationWithDetails> {
    const response = await apiClient.get<ProductApplication>(
      `/product-applications/${id}`,
    );
    ...
    const [propertyResponse, cultureResponse] = await Promise.all([
      apiClient
        .get<Property>(`/properties/${app.propertyId}`)
        .catch(() => null),
  ...
  async update(
    ...
    const response = await apiClient.patch<ProductApplication>(
      `/product-applications/${id}`,
      data,
    );
    return response.data;
