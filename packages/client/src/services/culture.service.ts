...
class CultureService {
  ...
  async findByProperty(propertyId: string): Promise<Culture[]> {
    try {
      ...
      console.log(`[CultureService] Buscando culturas para propertyId: ${propertyId}`);
      
      const response = await apiClient.get(`/cultures/by-property/${propertyId}`);
      console.log('[CultureService] Resposta do endpoint:', response.data);
      
      // O endpoint retorna array DIRETO (sem wrapper {data: [...]})
      if (Array.isArray(response.data)) {
        console.log(`[CultureService] Encontrou ${response.data.length} culturas`);
        return response.data;
      }
      
      // Se por algum motivo tiver wrapper (para compatibilidade futura)
      if (response.data && Array.isArray(response.data.data)) {
        console.log(`[CultureService] Encontrou ${response.data.data.length} culturas em data wrapper`);
  ...
  private async findAllAndFilter(propertyId: string): Promise<Culture[]> {
    try {
      ...
      console.log(`[CultureService] Fallback: ${filtered.length} culturas filtradas`);
      return filtered;
    } catch (fallbackError) {
