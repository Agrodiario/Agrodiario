...
export function CultureDetailsDrawer({ culture, onDelete }: Props) {
  ...
  const translatedOrigin = culture.origin
    ? originMap[culture.origin] || culture.origin
    : "N/A";

  // Formatar datas
  const formattedPlantingDate = culture.plantingDate
    ? format(parseDateSafe(culture.plantingDate), "dd 'de' MMMM 'de' yyyy", {
        locale: ptBR,
      })
    : "N/A";
  ...
  const formattedExpectedHarvest = culture.expectedHarvestDate
    ? format(
        parseDateSafe(culture.expectedHarvestDate),
        "dd 'de' MMMM 'de' yyyy",
        { locale: ptBR },
      )
    : "N/A";
  return (
    <div className={styles.container}>
      ...
      <div className={styles.mainContent}>
        ...
        <section className={styles.section}>
          ...
          <InfoBlock
            ...
            value={culture.cultivar || "N/A"}
          />
          <InfoBlock
            label="Propriedade"
            value={culture.property?.name || "N/A"}
          />
          {culture.plotName && (
            <InfoBlock label="Talhão" value={culture.plotName} />
          )}
          <InfoBlock
            label="Área de plantio (hectares)"
            value={culture.plantingArea}
          />
          <InfoBlock label="Ciclo (dias)" value={culture.cycle} />
          <InfoBlock
            label="Status"
            value={culture.isActive ? "Ativo" : "Não ativo"}
          />
        </section>
        ...
        <section className={styles.section}>
          ...
          <InfoBlock
            ...
            value={formattedExpectedHarvest}
          />
          <InfoBlock
            label="Dias decorridos"
            value={
              culture.daysElapsed !== undefined ? culture.daysElapsed : "N/A"
            }
          />
          <InfoBlock
            label="Dias restantes"
            value={
              culture.daysRemaining !== undefined
                ? culture.daysRemaining
                : "N/A"
            }
          />
        ...
        <section className={styles.section}>
          ...
          <p className={styles.activitiesInfo}>
            {culture.activitiesCount || 0} atividade(s) vinculada(s) a esta
            cultura.
          </p>
        </section>
      </div>
      ...
      >
        Tem certeza que deseja excluir esta cultura? Esta ação não pode ser
        desfeita.
      </ConfirmationModal>
    </div>
  );
