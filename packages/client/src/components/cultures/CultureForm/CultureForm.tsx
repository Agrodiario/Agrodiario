...
export function CultureForm({ initialData, onSubmit, isLoading = false }: Props) {
  ...
  const isFormValid = () => {
    ...
    if (!formData.cultureName || formData.cultureName.trim() === '') return false;
    ...
    if (!formData.plantingDate || formData.plantingDate.trim() === '') return false;
    
    // Validação da área de plantio (hectares ou talhão)
    if (areaInputType === 'hectares') {
      if (!formData.plantingArea || formData.plantingArea.trim() === '') return false;
          ...
          {areaInputType === 'hectares' ? (
            <>
              <Input
                ...
                step="0.01"
                min="0.01"
                max={selectedProperty ? (selectedProperty.productionArea || selectedProperty.totalArea) : undefined}
                value={formData.plantingArea}
                onChange={handleChange}
                required
              />
              {selectedProperty && formData.plantingArea && parseFloat(formData.plantingArea) > (selectedProperty.productionArea || selectedProperty.totalArea) && (
                <p style={{ fontSize: '0.85rem', color: '#d92d20', marginTop: '0.5rem' }}>
                  A área de plantio não pode ser maior que a área de produção da propriedade ({selectedProperty.productionArea || selectedProperty.totalArea} ha)
                </p>
              )}
            </>
          ) : (
