...
export function ActivityDetailsDrawer({ activity, onDelete }: Props) {
  ...
  const getTagColor = (type: string) => {
    ...
    if (
      normalizedType.includes("aplicacao") ||
      normalizedType.includes("aplicação")
    )
      return "blue";
      ...
      <div className={styles.mainContent}>
        <section className={styles.section}>
          ...
          <InfoBlock
            ...
            value={new Date(activity.date).toLocaleDateString("pt-BR")}
            icon={<BsCalendar4Event color="#006a35" />}
          />
          <InfoBlock
            label="Propriedade associada"
            value={activity.propriedade}
            icon={<RiHome8Line color="#006a35" size={18} />}
          />
