...
import { FiArrowLeft, FiUpload, FiEye, FiTrash2 } from "react-icons/fi";
...
export function ActivityForm({
  ...
  const [_insumoOptions, setInsumoOptions] = useState<
  ...
  const validateOtherFields = (
    fieldName: keyof ActivityFormData,
    value: string,
  ): string => {
    switch (fieldName) {
      case "titulo":
        if (!value || value.trim() === "")
          return "Título da atividade é obrigatório";
        return "";
      case "propriedade":
      ...
      case "responsavel":
        ...
        return "";
      case "insumoNome":
        if (showInsumos && (!value || value.trim() === ""))
          return "Nome do produto/insumo é obrigatório";
        return "";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const allRequiredFields: (keyof ActivityFormData)[] = [
      "date",
      "titulo",
      "propriedade",
      "cultureId",
    ...
    if (!formData.propriedade.trim())
      newErrors.propriedade = "Propriedade é obrigatória";
    if (!formData.cultureId.trim())
      newErrors.cultureId = "Cultura é obrigatória";
    ...
    if (showInsumos) {
      const insumoNomeError = validateOtherFields(
        "insumoNome",
        formData.insumoNome || "",
      );
      if (insumoNomeError) newErrors.insumoNome = insumoNomeError;

      const insumoQuantidadeError = validateNumberField(
        formData.insumoQuantidade || "",
        "Quantidade utilizada",
      );
      if (insumoQuantidadeError)
        newErrors.insumoQuantidade = insumoQuantidadeError;
    }
  ...
  useEffect(() => {
    async function fetchEmbrapaInputs() {
      ...
      try {
        ...
        console.error(
          "Erro ao carregar insumos Embrapa (usando fallback):",
          error,
        );

        setInsumoOptions([
          {
            label: "Fertilizante NPK 10-10-10 (Offline)",
            value: "Fertilizante NPK 10-10-10",
          },
          { label: "Glifosato (Offline)", value: "Glifosato" },
          {
            label: "Inseticida Cipermetrina (Offline)",
            value: "Inseticida Cipermetrina",
          },
        ]);
      } finally {
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.tagGroup}>
            <TagToggle
              color="red"
              isActive={formData.tipo === "preparo"}
              onClick={() => handleTypeChange("preparo")}
              type="button"
            >
              Preparo
            </TagToggle>
            <TagToggle
              color="blue"
              isActive={formData.tipo === "aplicacao"}
              onClick={() => handleTypeChange("aplicacao")}
              type="button"
            >
              Aplicação
            </TagToggle>
            <TagToggle
              color="green"
              isActive={formData.tipo === "colheita"}
              onClick={() => handleTypeChange("colheita")}
              type="button"
            >
              Colheita
            </TagToggle>
            <TagToggle
              color="orange"
              isActive={formData.tipo === "manejo"}
              onClick={() => handleTypeChange("manejo")}
              type="button"
            >
              Manejo de solo
            </TagToggle>
